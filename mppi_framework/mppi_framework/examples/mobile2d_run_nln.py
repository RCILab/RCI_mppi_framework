...
def main():
    ...
    p.add_argument("--save", type=str, default="outputs/mobile2d_nln.gif")
    args = p.parse_args()
    ...
    cfg = MPPIConfig(
        ...
        u_max=args.u_max,          # v, omega are clipped separately inside dynamics
        device=args.device,
        dtype=torch.float32,
    ...
    ctrl = build_controller(
        ...
        sampler_cfg={
            "std_n": args.std_init, "std_ln":[0.1,0.1], "mu_ln": [1.0,0.2], "device": args.device
        },
    )


    # 2) Initial state: [x, y, theta]
